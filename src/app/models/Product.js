...
Product.pre('save', function (next) {
    if (this.isModified('nameProduct')) {
        // Kiểm tra nếu trường name được cập nhật
        this.slug = slugify(this.nameProduct + '-' + Math.random().toString(36).substr(2, 6), {
            lower: true,
            strict: true,
            replacement: '-',
            trim: true,
...
Product.pre('updateOne', function (next) {
    ...
    if (update.nameProduct) {
        // Kiểm tra nếu trường name được cập nhật
        update.slug = slugify(update.nameProduct + '-' + Math.random().toString(36).substr(2, 6), {
            lower: true,
            strict: true,
            replacement: '-',
            trim: true,
